      ...
      <nav className="flex-1 overflow-y-auto p-3 flex flex-col gap-1">
        {links.map(({ href, label, icon: Icon, exact }) => {
          ...
          return (
            ...
            >
              <Icon size={16} className={active ? "text-secondary" : "text-base-content/40 group-hover:text-base-content/70"} />
...
const DashboardSidebar = ({ user }) => {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex flex-col w-64 shrink-0 bg-base-100 border-r border-base-300 sticky top-16 h-[calc(100vh-4rem)]">
      ...
      {open && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <aside className="relative w-72 bg-base-100 border-r border-base-300 flex flex-col shadow-2xl">
            ...
            <SidebarBody user={user} links={links} onLinkClick={() => setOpen(false)} onLogout={handleLogout} />
          </aside>
        </div>
      )}
    </>
